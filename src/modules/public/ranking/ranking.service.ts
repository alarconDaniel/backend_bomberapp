...
/**
 * Entrada del TOP de ranking por XP.
 */
type TopItem = {
  codUsuario: number;
  nombre: string;
...
/**
 * Resumen de la posición del usuario en el ranking.
 */
type MeInfo = {
  position: number;
  xp: number;
...
/**
 * Trofeo visible en el ranking, con su portador actual (si existe).
 */
type TrophyItem = {
  codTrofeo: number;
  nombre: string;
...
export class RankingService {
  ...
  /**
   * Lee XP_POR_NIVEL desde config o usa un valor seguro por defecto.
   */
  ...
  /**
   * Calcula la posición global del usuario por XP (1-based).
   * La posición se define como: #personas con más XP + 1.
   */
  ...
  /**
   * Lista todos los trofeos y su portador actual (si lo hay).
   */
