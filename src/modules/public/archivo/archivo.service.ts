import {
  ...
  // Entidades (ajusta rutas si difieren)
  ...
  /** Convierte un file de Drive a tu forma común */
  ...
  export class ArchivoService {
    ...
    async subirDesdeBackend(
      ...
      // Permitir descarga sin OAuth (public read)
      await this.ensurePublicReadable(fileId, drive);
  
      // Guardar metadatos en BD (opcional pero consistente con tu modelo)
      const usuario = await this.asegurarUsuario(codUsuario);
      const entity = this.archivoRepo.create({
        nombreOriginal: nombre,
        path: fileId, // IMPORTANTE: ahora path = fileId de Drive
        contentType: res.data.mimeType || archivo.mimetype,
        sizeBytes: Number(res.data.size || archivo.size || 0),
    ...
    private async ensurePublicReadable(fileId: string, drive: drive_v3.Drive) {
      try {
        ...
        // Ignora si ya existe o no se puede repetir
    ...
    async eliminarPorPath(path: string, codUsuario: number) {
      ...
      // Borra en Drive (path es el fileId)
      try {
